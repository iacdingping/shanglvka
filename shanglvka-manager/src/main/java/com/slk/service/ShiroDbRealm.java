...
import com.slk.core.entity.sys.UserState;
...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if (user != null) {
			UserState us = user.getUserState(token.getHost());
			if (us != UserState.ENABLED) {
				throw new DisabledAccountException(us.getDescription());
			}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		List<Role> roles = userService.getRoleByUser(user.getId());
		for (Role role : roles) {
			// 基于Role的权限信息
			info.addRole(role.getName());
			// 基于Permission的权限信息
			info.addStringPermissions(userService.getRolePemissions(role));
		}
		return info;
	}
