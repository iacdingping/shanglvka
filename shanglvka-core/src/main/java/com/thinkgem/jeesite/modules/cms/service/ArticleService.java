...
public class ArticleService extends BaseService {
	...
	public Page<Article> find(Page<Article> page, Article article,
		...
		if (article.getCreateBy() != null
				&& StringUtils.isNotBlank(article.getCreateBy().getId())) {
			dc.add(Restrictions
	...
	@Transactional(readOnly = false)
	public void save(Article article, String[] votingTitle) {
		...
		// 删除之前的投票
		votingDao.deleteAllByArticleId(article.getId());
		if (votingTitle != null && votingTitle.length > 0) {
			List<Voting> votings = new ArrayList<Voting>();
			for (String str : votingTitle) {
				votings.add(new Voting(str, article.getId()));
			}
			votingDao.save(votings);
		}
	}
	...
	public Page<Article> search(Page<Article> page, String q,
		...
		bcList.add(new BooleanClause(new TermQuery(new Term(
				Article.FIELD_DEL_FLAG, Article.DEL_FLAG_NORMAL)), Occur.MUST));
