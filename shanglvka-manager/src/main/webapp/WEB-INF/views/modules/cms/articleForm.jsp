...
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						...
						$("#posidListArea").find("input[name='posidList']")
								.click(function() {
	...
	function deleteVoting(num) {
		$("#voting"+num).remove();
	}
	
	function addVoting() {
		var num = $("#votingNum").val();
		num++;
		$("#votingNum").val(num);
		$("#votingDiv")
				.append(
						"<div id='voting"
								+ num
								+ "'  class='control-group'><label class='control-label'>投票"
								+ num
								+ ":</label><div class='controls'><input name='votingTitle' type='text' class='input-xlarge required'> <input type='button' value='删除' onclick='deleteVoting("
									+ num
									+ ");'></div></div>");
	}
</script>
...
<body>
	...
	<br />
	<input id="votingNum" type="hidden" value="${fn:length(votings)}">
	<form:form id="inputForm" modelAttribute="article"
		...
		<div class="control-group">
			...
			<div class="controls">
				<tags:treeselect id="category" name="category.id"
		...
		<div class="control-group">
			<label class="control-label">缩略图(<span id="image_size_id">50X50</span>):
			</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label">投票可选数:</label>
			<div class="controls">
				<form:input path="optionalNum" htmlEscape="false" maxlength="3"
					class="required" />
			</div>
		</div>
		<div class="control-group">
			<label class="controls"> <input type="button" value="增加投票"
				onclick="addVoting();">
			</label>
		</div>
		<div id="votingDiv">
			<c:forEach var="item" items="${votings}" varStatus="votings">
				<div id="voting${votings.count}" class='control-group'>
					<label class='control-label'>投票${votings.count}:</label>
					<div class='controls'>
						<input name='votingTitle' type='text'
							class='input-xlarge required' value="${item.title}"> <input
							type="button" value="删除"
							onclick="deleteVoting(${votings.count});">
					</div>
				</div>
			</c:forEach>
		</div>
